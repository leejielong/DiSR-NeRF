...
class NIQE(nn.Module):
    ...
    def forward(self, img):
        img = torch.flip(img, dims=[-1]) # convert rgb to bgr
        img = img[0].detach().cpu().numpy() * 255.0
        niqe = calculate_niqe(img=img, crop_border=0, input_order='HWC')
        return torch.tensor(niqe,dtype=torch.float32)
